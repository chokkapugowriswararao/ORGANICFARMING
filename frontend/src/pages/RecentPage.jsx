...
const RecentCustomers = () => {
  ...
  const fetchCustomers = async () => {
    setLoading(true);
    try {
      const response = await axios.get('http://localhost:5000/api/customers/recent', {
        withCredentials: true,
      });
      setCustomers(response.data);
      setLoading(false);
    } catch (err) {
      setError('Error fetching recent customers');
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto my-8">
      <h2 className="text-4xl font-bold text-center mb-6">Recent Customers</h2>
        ...
        {customers.slice(0, visibleCount).map((customer) => (
          <div key={customer._id} className="card w-full bg-base-100 shadow-xl hover:shadow-2xl transition-shadow">
            <div className="card-body">
              ...
              <p className="text-red-700">Id: {customer._id}</p>
